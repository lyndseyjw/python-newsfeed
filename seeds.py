...
# need to establish a temp session connection with Session class anytime performing a CRUD operation
db = Session()

# insert users (this only preps SQL queries)
db.add_all([
...
# this runs INSERT statements
db.commit()

# insert posts (needs to insert after users are committed because requires valid user_id fields)
db.add_all([
...
# close session connection when finished making db transactions
db.close()